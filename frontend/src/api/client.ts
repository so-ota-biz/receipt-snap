...
import type { UploadAndClassifyResponse, OcrRequest, OcrResponse, AnalysisLog } from '../types/api'
 ...
 * 直近のログを取得する
 */
export const getRecentLogs = async (limit: number = 10): Promise<AnalysisLog[]> => {
  const response = await axios.get<AnalysisLog[]>(`${API_BASE_URL}/api/logs/recent`, {
    params: { limit },
  })
  return response.data
}
